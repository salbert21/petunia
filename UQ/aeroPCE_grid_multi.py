...
sigmaList = [0.01, 0.001]
NfactList = np.array([0.2, 0.5, 0.9, 1.1, 4])

nt = 20 # number of trials to average over for each gridpoint
...
for sigma in sigmaList:
    for Nfact in NfactList:
        ...
        for i in range(nt):
            C_SPGi, mean_SPGi, var_SPGi, infi = solveSPGL1(Psi, u_samps,
                                                           N, N_SPG, sigma)
            C_SPGis.append(C_SPGi)
            mean_SPGis.append(mean_SPGi)
            var_SPGis.append(var_SPGi)
            niterslist.append(infi['niters'])
            
        C_SPG = np.mean(C_SPGis)
        mean_SPG = np.mean(mean_SPGis)
        var_SPG = np.mean(var_SPGis)
        niters = np.mean(niterslist)
        ...
        nitersList.append(niters)
        
        # =====================================================================
        # Print
        # =====================================================================
        print('\n\ncompleted all runs for Psi file: ' + Psifilename)
        print('\nsigma = {}'.format(sigma))
...
C_SPGList.extend(nanlist)
mean_SPGList.extend(nanlist)
meanErrList.extend(nanlist)
var_SPGList.extend(nanlist)
varErrList.extend(nanlist)
runtimeList.extend(nanlist)
nitersList.extend(nanlist)
...
if filtered:
    outname = './results/' + QOI + 'res_multi_p' + str(p) + '_FILT_' + MCname
else:
    outname = './results/' + QOI + 'res_multi_p' + str(p) + '_' + MCname
